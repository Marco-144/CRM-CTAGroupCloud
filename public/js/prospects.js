(() => {
    ...
    function renderTable(data) {
        ...
        const isMobile = isMobileVerticalView();
        ...
        if (isMobile) {
            ...
            prospectsCards.innerHTML = data.map((prospect) => {
                ...
                <div class="prospect-card">
                    <h6>${escapeHTML(prospect.name)}</h6>
                    ...
                    <div class="prospect-meta">
                        ✉️ ${escapeHTML(prospect.email || '-')}
                    </div>
                    <div class="mt-2">
                        ${getPriority(escapeHTML(prospect.priority))}
                        <span class="${statusClass} ms-2">${escapeHTML(prospect.status)}</span>
                    </div>
                    <div class="prospect-actions">
                        <button class="btn btn-sm btn-outline-secondary edit-prospect"
    ...
    if (prospectForm && prospectsTable && searchInput && statusFilter) {
        ...
        prospectsCards?.addEventListener('click', async (event) => {
            const editBtn = event.target.closest('.edit-prospect');
            if (editBtn) {
                loadProspectsInForm(editBtn.dataset.id);
                return;
